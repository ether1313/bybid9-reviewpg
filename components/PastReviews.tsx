...
export default function PastReviews() {
  ...
  const formatReview = (r: any): Review => ({
    ...
    wallet: r.casino_wallet || r.wallet || 'Bybid9',
  ...
  const updateVirtualStats = () => {
    const baseDate = new Date('2025-11-06T08:00:00Z')
    const now = new Date()
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('realtime:bybid9_review')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'bybid9_review' }, (payload) => {
            ...
            >
              {reviews.map((review, index) => (
                <div key={review.id || index} className="relative min-w-[20%] max-w-[20%] flex-shrink-0">
                  {index === 0 && (
                    <span className="absolute -top-4 right-4 z-30 bg-gradient-to-r from-[#9cd6ff] to-[#cfe9ff] text-black text-xs font-semibold px-4 py-1.5 rounded-full shadow-md animate-pulse">
                      Latest Review
                    </span>
                  )}
          ...
          >
            {reviews.slice(0, expandedIndex === -1 ? 10 : 5).map((review, index) => (
              <div key={review.id || index} id={`review-${index + 1}`} className="relative transition-all duration-500">
                {index === 0 && (
                  <span className="absolute -top-3 right-4 z-30 bg-gradient-to-r from-[#9cd6ff] to-[#cfe9ff] text-black text-[11px] font-semibold px-3 py-1 rounded-full shadow-md animate-pulse">
                    Latest Review
                  </span>
                )}
